...
## Підсумок
- **Усього сторінок:** 60
- **Асетів:** 642 (~174 MB) з Webflow-CDN + 1124 файли карти (~50 MB) — усе локально
...
**Реалізація:** інтерактивна **Leaflet 1.9.4** (custom-code embed) — тайлована піраміда зі скану плану Вінниці 1945 р. (15942×13303 px, зум 0–5, схема TMS).

**Усе self-hosted (з 2026-07-23):**
- Leaflet js/css + іконки маркерів → `static/leaflet/`
- 1117 тайлів (z0–z5) → `static/map-tiles/{z}/{x}/{y}.png` (~47 MB)
- Раніше вантажилось із `unpkg.com` і `gyborg.github.io` — **більше жодних зовнішніх залежностей у карти немає.**

12 маркерів ведуть на сторінки `map/…`. Тайли побайтово ідентичні оригіналу; 404 на позаграничних тайлах (напр. `2/-1/1.png`) — нормальна поведінка Leaflet, ідентична оригіналу.

**Оновлення тайлів:** `python3 scripts/fetch_map_assets.py` (ідемпотентний). `static/` не видаляється при перезбірці (`build.py` → PRESERVE).
